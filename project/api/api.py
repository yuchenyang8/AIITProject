...
from extensions.ext import NmapExt, WafExt, WappExt, BinExt, OneForAllExt, MobExt, PocExt, HydraExt, XrayExt, DirExt, NessusExt, DOCS, SUPPORT_PROTOCOL, POC_DIR, CONFIG_PATH
from project.models.base import DB
from utils.auxiliary import get_title, modify_yaml, api_required
    ...
    def get(info_type, company, asset_type):
        ...
        if info_type == 'distribute':
            ...
            for i in range(-6, 1, 1):
                ...
                result.append({'date': day, 'host': hostvulns, 'web': webvulns, 'project': appvulns, 'firm': firmvulns})
            return result
        ...
        try:
            ...
            if file_data:
                if file_data.filename.split('.')[-1] in ['bin']:
                    ...
                    file_data.save(self.path + 'project\\' + secure_filename(file_data.filename))
                    return {'code': 200, 'msg': '上传成功！'}
            else:
    ...
    def post(self):
        ...
        if task_type == '主机':
            ...
            for asset_name in task_info:
                ...
                dir_list = DirExt(asset_name).scan()
                ...
                subdomain_list = OneForAllExt(asset_name).discovery()
                DB.db.asset.update_one({'aname': asset_name}, {
                ...
                for subdomain in subdomain_list:
                    ...
                    subdomain_dir_list = DirExt(subdomain).scan()
                    DB.db.asset.update_one({'aname': subdomain},
...
class CaseAPI(Resource):
    ...
    def post(self):
        ...
        for i in range(5):
            executor.submit(self.u, i)
        return {'status_code': 200, 'msg': 'GEVENT!'}
