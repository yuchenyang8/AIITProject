...
import bson
from flask import request, session, redirect, url_for, render_template, send_from_directory, Blueprint

from project.models.base import DB
from utils.auxiliary import kill_process, get_yaml
from utils.auxiliary import login_required, admin_required

aiit = Blueprint('aiit', __name__, template_folder='templates')


@aiit.route('/func/company')
@admin_required
...
@aiit.route('/func/asset/<string:asset_name>')
@login_required
...
@aiit.route('/func/poc/task/<string:objid>')
@login_required
...
@aiit.route('/func/vulns/<string:vuln_type>')
@login_required
...
@aiit.route('/func/vulns/host/<string:objid>')
@login_required
...
@aiit.route('/func/vulns/web/<string:objid>')
@login_required
...
@aiit.route('/func/company/<string:company_name>')
@admin_required
def html_func_company_info(company_name):
    ...
    DB.db.company.update_one({'ename': company_name}, {
        '$set': {'count.host': hostcount, 'count.web': webcount, 'count.project': appcount,
...
@aiit.route('/func/webhook', methods=['POST'])
def xray_webhook():
    data = request.json
...
@aiit.route('/extmanage/<string:name>')
@admin_required
...
@aiit.route("/download/<filename>", methods=["GET"])
@admin_required
